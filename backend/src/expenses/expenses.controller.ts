...
export function createExpensesController(expensesService: ExpensesService) {
  return {
    ...
    async getSupportedCurrencies(req: Request, res: Response) {
      const result = await expensesService.getSupportedCurrencies(req.actor.organizationId);
      res.json(result);
    },

    async getSupportedLanguages(req: Request, res: Response) {
      const result = await expensesService.getSupportedLanguages(req.actor.organizationId);
      res.json(result);
    },
